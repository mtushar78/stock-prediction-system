...
async def scheduled_data_update():
    ...
    try:
        ...
        # Update stock data from stocksurferbd
        try:
            fetcher = StockSurferFetcher(db)
            stats = fetcher.update_all_tickers(delay=2.0)
...
async def lifespan(app: FastAPI):
    ...
    # Schedule 3 updates during market hours (10:00 AM - 2:30 PM)
    # 1. Opening update at 10:15 AM (after market opens)
    scheduler.add_job(
        scheduled_data_update,
        CronTrigger(hour=10, minute=15, timezone=BANGLADESH_TZ),
        id='opening_update',
        name='Opening Data Update',
        replace_existing=True
    )
    
    # 2. Mid-day update at 12:00 PM
    scheduler.add_job(
        scheduled_data_update,
        CronTrigger(hour=12, minute=0, timezone=BANGLADESH_TZ),
        id='midday_update',
        name='Mid-Day Data Update',
        replace_existing=True
    )
    
    # 3. Closing update at 2:45 PM (after market closes at 2:30 PM)
    scheduler.add_job(
        scheduled_data_update,
        CronTrigger(hour=14, minute=45, timezone=BANGLADESH_TZ),
        id='closing_update',
        name='Closing Data Update',
        replace_existing=True
    )
    
    scheduler.start()
    logger.info("⏰ Scheduler started: 3 updates during market hours")
    logger.info("  - Opening: 10:15 AM")
    logger.info("  - Mid-Day: 12:00 PM")
    logger.info("  - Closing: 2:45 PM")
    
    # Get next run times
    for job_id in ['opening_update', 'midday_update', 'closing_update']:
...
def health_check() -> SystemStatus:
    ...
    try:
        ...
        next_update = None
        if job and job.next_run_time:
            next_update = job.next_run_time.strftime('%Y-%m-%d %H:%M:%S %Z')
