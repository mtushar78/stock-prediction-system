...
/**
 * ManualAnalysisView — the FULL manual-analysis breakdown for one ticker,
 * extracted from /analyze so it can be reused verbatim inside a modal (e.g. the
 * Rebounds page) as well as on the standalone page.
 *
 * Given a ticker it fetches the detailed analyzer output + 60-day score history
 * and renders every section the /analyze page shows: breakout verdict, score
 * history, sector/fundamentals, the collapsed legacy score engine, and the full
 * chart analysis (Bulkowski patterns + candlesticks + geometry).
 *
 * It owns its own loading/error state and refetches whenever `ticker` changes.
 */
...
import { ChartAnalysisBody } from './ChartDetailModal';
...
export default function ManualAnalysisView({ apiUrl, ticker }: Props) {
  ...
  useEffect(() => {
    ...
    axios
      .post<DetailedTickerAnalysis>(`${apiUrl}/api/analyze-ticker`, { ticker: t })
      ...
      .finally(() => { if (!cancelled) setLoadingAnalyze(false); });

    axios
      .get<ScoreHistory>(`${apiUrl}/api/score-history/${t}?days=60`)
    ...
    <div>
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <div className="text-xs text-gray-500 mt-1">Detailed breakdown for {ticker.toUpperCase()}</div>
        </div>
        <div className="flex items-center gap-2">
          {statusBadge}
          {breakoutBadge}
      ...
      {loadingAnalyze && !result && <div className="text-gray-400 text-sm mt-6">Analyzing {ticker.toUpperCase()}…</div>}
      ...
      {result && (
        <div className="mt-6 space-y-6">
          ...
          <div className="bg-gray-950 border border-purple-800/40 rounded p-1 pt-4">
            ...
            <ChartAnalysisBody apiUrl={apiUrl} ticker={result.ticker} />
          </div>
        </div>
      )}
    </div>
